...
if isWin:
    ...
    mplayerCmd += ["-vo", "direct3d", "-ao", "win32"]
    ...
    def startProcess(self):
        try:
            ...
            if not wid_mplayer_container:
                return
            try:                
                volume = int(mcon.get_gs("mplayer_startup_volume"))
            except Exception:
                volume = 50
...
def queueMplayer(path, start_sec_p=0, end_sec_p=0):
    ...
    if isWin and os.path.exists(path):
        ...
        if mcon.key_in_gs("my_mplayer_need_copying_file_to_temp") \
                and mcon.get_gs("my_mplayer_need_copying_file_to_temp"):
            dir = tmpdir()
...
def pause():
    ...
    try:
        mplayerManager.mplayer.stdin.write(b"pause\n")
        mplayerManager.mplayer.stdin.flush()        
    except:
        pass
...
def seek(sec : int):
    global mplayerManager
    try:
        mplayerManager.mplayer.stdin.write(f"seek {sec} 0\n".encode("utf-8"))
        mplayerManager.mplayer.stdin.flush()        
    except:
        pass    

def volume_change(delta: int):
    try:
        mplayerManager.mplayer.stdin.write(f"volume {delta} 0\n".encode("utf-8"))
        mplayerManager.mplayer.stdin.flush()
    except Exception:
        pass
...
def play(path, start_sec=0, end_sec=0):
    ...
    if mplay_readiness_state == 'failed and notified':
        ...
        loc1 = mcon.get_loc("Mplayer_Missing", "Mplayer Missing!")
        loc2 = mcon.get_loc("Unable_to_open_mplayer", "Unable to open mplayer. \nHave you installed mplayer?\nOn Windows, you can copy mplayer.exe from the Anki version 2.1.26\ninstallation to the location of the anki.exe file\n(...AppData\\Local\\Programs\\Anki\\)")        
        QMessageBox.critical( None, loc1, loc2 )            
        completed_play_notice.set()
